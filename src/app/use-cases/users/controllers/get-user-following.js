// controllers/users/getUserFollowing.js
const mongoose = require("mongoose");
const User = require("../../../models/User");

/**
 * Lista os usuários que um determinado usuário (userId, na URL) está seguindo.
 * Respeita bloqueios e privacidade do perfil, igual ao "seguindo" do Instagram.
 */
const getUserFollowing = async (req, res) => {
  try {
    const viewerId = req?.user?.id; // quem está autenticado e olhando a lista
    const userId = req.params?.user_id; // dono da lista de "seguindo"
    ...
    // Se o dono da lista me bloqueou, a lista simplesmente "não existe" pra mim
    const targetBlockedViewer = (targetUser.blocked_users || []).some((id) => id.equals(viewerObjectId));
    if (targetBlockedViewer) {
      return res.status(404).json({ message: "Usuário não encontrado." });
    }

    // Perfil privado: só quem segue (ou o próprio dono) pode ver a lista
    ...
    // Começa pelo próprio documento do usuário-alvo, mantendo a ordem original
    // do array "following" (mais recentes por último = mais recentes primeiro na listagem).
    const basePipeline = [
      { $match: { _id: targetObjectId } },
      { $project: { list: "$following" } },
